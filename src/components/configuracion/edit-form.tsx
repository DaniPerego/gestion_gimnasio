...
import { ConfiguracionDB, Configuracion } from '@/lib/db';
...
export default function ConfigForm({ config }: { config: Configuracion | null }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const existing = ConfiguracionDB.findFirst();
      if (existing) {
        ConfiguracionDB.update({ id: existing.id }, {
          nombreGimnasio: nombreGimnasio.trim(),
          colorPrimario,
          colorSecundario,
          logoUrl: fondoUrl || null,
        });
      } else {
    ...
    <>
      <form onSubmit={handleSubmit}>
        <div className="rounded-md bg-gray-50 dark:bg-gray-800 p-4 md:p-6">
          ...
          {message && (
            <p className={`mt-2 text-sm ${message.includes('correctamente') ? 'text-green-600' : 'text-red-500'}`}>
              {message}
            </p>
          )}
        </div>
      ...
      <div className="mt-8 pt-8 border-t border-gray-200 dark:border-gray-700">
        ...
        <div>
          ...
          <form
            ...
            onSubmit={(e) => {
              if (!window.confirm('¿Estás SEGURO de que deseas importar este backup? Esto ELIMINARÁ todos los datos actuales.')) {
                e.preventDefault();
              }
            }}
          >
